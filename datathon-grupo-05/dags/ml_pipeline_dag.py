...
for config in TICKERS:
    ...
    with DAG(
        ...
        t1_coletar = BashOperator(
            ...
            bash_command=(
                f"cd {PROJECT_DIR} && "
                f"poetry run python src/features/data_collection.py --ticker {ticker}"
            ),
            doc_md=f"""
        ...
        t2_processar = BashOperator(
            ...
            bash_command=(
                f"cd {PROJECT_DIR} && "
                f"poetry run python src/features/feature_engineering.py --ticker_id {ticker_id}"
            ),
            doc_md="""
        ...
        t4_treinar = BashOperator(
            ...
            bash_command=(
                f"cd {PROJECT_DIR} && "
                f"poetry run python src/models/train.py --ticker_id {ticker_id}"
            ),
            doc_md="""
        ...
        t5_drift = BashOperator(
            ...
            bash_command=(
                f"cd {PROJECT_DIR} && "
                f"poetry run python src/monitoring/drift.py --ticker_id {ticker_id}"
            ),
            doc_md="""
