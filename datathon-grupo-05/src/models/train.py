...
Resolve:
    ...
    - GAP 01: Métricas financeiras rastreáveis (RMSE, MAE, MAPE) por epoch
...
def compute_financial_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    ...
    Returns:
        Dicionário com RMSE, MAE e MAPE.
    """
    mae  = np.mean(np.abs(y_true - y_pred))
    rmse = np.sqrt(np.mean((y_true - y_pred) ** 2))
    # MAPE: evita divisão por zero com clip
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % 10 == 0 or epoch == 0:
                logger.info(
                    "Epoch [%d/%d] | Train MSE: %.6f | Val RMSE: %.6f | MAE: %.6f | MAPE: %.2f%%",
                    epoch + 1, args.epochs,
                    avg_train_loss, fin_metrics["rmse"],
                    fin_metrics["mae"], fin_metrics["mape"],
                )

        logger.info("Treinamento concluído.")

        # === Persistência com assinatura automática (elimina WARNING do MLflow) ===
        input_example = X_train[:1].numpy()  # 1 amostra para inferência de signature
